...
--QUERY: Show each product and how many times it was ordered.
...
--QUERY: List the top 2 highest-paying customers (by total amount spent).
...
--QUERY: Show the running total of revenue (by payment date).
    
SELECT PAYMENT_DATE, AMOUNT, SUM(AMOUNT) OVER (ORDER BY PAYMENT_DATE) AS RUNNING_TOTAL FROM PAYMENTS 

--QUERY: Find customers who have never made a payment
...
--QUERY: Rank customers by total amount spent (include ties).
...
--QUERY: Find products that have been ordered but never delivered.
